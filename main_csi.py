...
if trained == True:
    ...
    encoder_t.load_state_dict(torch.load('encoder_s_csi.pkl'))
else :
    for e in range(50):
        fun.train_src(encoder_s,loader_source,opt_es)

    encoder_t.load_state_dict(encoder_s.state_dict())
    torch.save(encoder_s.state_dict(), 'encoder_s_csi.pkl')
acc = fun.validate(encoder_s,encoder_s,loader_source,loader_source)

goal = 0

for i in range(EPOCH):
    # fun.train_disc(encoder_s,encoder_t,disc,loader_train_source,loader_train_target,opt_et,opt_dis,EPOCH = 3)
    fun.fit_disc(encoder_s,encoder_t,disc,loader_source,loader_target,opt_et,opt_dis,20)
